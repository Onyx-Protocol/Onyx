...
func NewOutPoint(hash *Hash32, index uint32) *OutPoint {
	return &OutPoint{
		Hash:  *hash,
		Index: index,
	}
...
func (o OutPoint) String() string {
	...
	buf := make([]byte, 2*len(Hash32{})+1, 2*len(Hash32{})+1+10)
...
func (t *TxOut) SerializeSize() int {
	...
	// PkScript bytes + 20 asset id bytes.
	return 8 + VarIntSerializeSize(uint64(len(t.PkScript))) + len(t.PkScript) + len(t.AssetID)
}

// NewTxOut returns a new bitcoin transaction output with the provided
// transaction value and public key script.
func NewTxOut(assetID Hash20, value int64, pkScript []byte) *TxOut {
	return &TxOut{
		AssetID:  assetID,
		Value:    value,
		PkScript: pkScript,
...
// TxSha generates the Hash32 name for the transaction.
func (msg *MsgTx) TxSha() Hash32 {
...
func (msg *MsgTx) BtcDecode(r io.Reader, pver uint32) error {
	...
	if count > uint64(maxTxInPerMessage) {
		str := fmt.Sprintf("too many input transactions to fit into "+
			...
			maxTxInPerMessage)
		return errors.New("MsgTx.BtcDecode: " + str)
	}
...
func readTxIn(r io.Reader, pver uint32, version int32, ti *TxIn) error {
	...
	ti.SignatureScript, err = readVarBytes(r, pver, "transaction input signature script")
	if err != nil {
		return err
	}
...
func readTxOut(r io.Reader, pver uint32, version int32, to *TxOut) error {
	_, err := io.ReadFull(r, to.AssetID[:])
	if err != nil {
		return err
	}
	...
	to.PkScript, err = readVarBytes(r, pver, "transaction output public key script")
	if err != nil {
		return err
	}
