...
func main() {
	...
	contracts, err := ivy.Compile(os.Stdin)
	if err != nil {
		log.Fatal(err)
	}
	...
	for _, contract := range contracts {
		...
		fmt.Fprintf(buf, "\t_contractParams := []ivy.Param{\n")
		for _, param := range contract.Params {
			fmt.Fprintf(buf, "\t\t{Name: \"%s\", Type: \"%s\"},\n", param.Name, param.Type)
		}
		fmt.Fprintf(buf, "\t}\n")
		fmt.Fprintf(buf, "\tvar _contractArgs []ivy.ContractArg\n")
		for _, param := range contract.Params {
			switch param.Type {
			case "Amount":
				fmt.Fprintf(buf, "\t_%s := int64(%s)\n", param.Name, param.Name)
				fmt.Fprintf(buf, "\t_contractArgs = append(_contractArgs, ivy.ContractArg{I: &_%s})\n", param.Name)
			case "Asset":
				fmt.Fprintf(buf, "\t_%s := %s[:]\n", param.Name, param.Name)
				fmt.Fprintf(buf, "\t_contractArgs = append(_contractArgs, ivy.ContractArg{S: &_%s})\n", param.Name)
			case "Boolean", "Hash", "Program", "PublicKey", "Signature", "String":
				fmt.Fprintf(buf, "\t_contractArgs = append(_contractArgs, ivy.ContractArg{S: &%s})\n", param.Name)
			case "Integer":
				fmt.Fprintf(buf, "\t_contractArgs = append(_contractArgs, ivy.ContractArg{I: &%s})\n", param.Name)
			case "Time":
				fmt.Fprintf(buf, "\t_%s := %s.UnixNano() / time.Millisecond\n", param.Name, param.Name)
				fmt.Fprintf(buf, "\t_contractArgs = append(_contractArgs, ivy.ContractArg{I: &_%s})\n", param.Name)
			}
		}
		fmt.Fprintf(buf, "\treturn ivy.Instantiate(_contractParams, %s_body_bytes, %v, _contractArgs)\n", contract.Name, contract.Recursive)
		fmt.Fprintf(buf, "}\n\n")
...
func paramsStr(params []*ivy.Param) string {
	var strs []string
...
func asGoParams(params []*ivy.Param) (goParams string, imports []string) {
	var strs []string
	for _, p := range params {
		var typ string
		switch p.Type {
