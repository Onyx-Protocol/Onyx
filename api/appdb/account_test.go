...
func TestAccountBalance(t *testing.T) {
	const sql = `
		...
		VALUES ('t0', 0, 'a1', 10, 0, 'acc0', 'mn1'),
		       ('t1', 1, 'a1', 5, 0, 'acc0', 'mn1'),
		       ('t2', 2, 'a2', 20, 0, 'acc0', 'mn1');
	`
	withContext(t, sql, func(t *testing.T, ctx context.Context) {
		...
		for _, c := range cases {
			got, gotLast, err := AccountBalance(ctx, c.bID, c.prev, c.limit)
			if err != nil {
				t.Errorf("AccountBalance(%s, %s, %d): unexpected error %v", c.bID, c.prev, c.limit, err)
				continue
			}

			if !reflect.DeepEqual(got, c.want) {
				t.Errorf("AccountBalance(%s, %s, %d) = %v want %v", c.bID, c.prev, c.limit, got, c.want)
			}

			if gotLast != c.wantLast {
				t.Errorf("AccountBalance(%s, %s, %d) last = %v want %v", c.bID, c.prev, c.limit, gotLast, c.wantLast)
			}
		}
	})
}
