...
func TestConflictingTxsInPool(t *testing.T) {
	...
	// Build the second tx
	secondTemplate, err := Build(ctx, &tx.TxData, []Action{&SetTxRefDataAction{[]byte("test")}}, time.Now().Add(time.Minute))
	if err != nil {
		t.Fatal(err)
	}
...
func issue(ctx context.Context, t testing.TB, c *protocol.Chain, info *clientInfo, destAcctID string, amount uint64) (*bc.Tx, error) {
	...
	issueTx, err := Build(ctx, nil, []Action{assettest.NewIssueAction(assetAmount, nil), issueDest}, time.Now().Add(time.Minute))
	if err != nil {
		return nil, err
	}
