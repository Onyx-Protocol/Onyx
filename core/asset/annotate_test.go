...
func TestAnnotateTxs(t *testing.T) {
	...
	asset1, err := Define(ctx, []string{testutil.TestXPub.String()}, 1, nil, bc.Hash{}, nil, tags1, nil)
	if err != nil {
		t.Fatal(err)
	}

	tags2 := map[string]interface{}{"foo": "baz"}
	asset2, err := Define(ctx, []string{testutil.TestXPub.String()}, 1, nil, bc.Hash{}, nil, tags2, nil)
	if err != nil {
		t.Fatal(err)
	}
