...
func TestHistoricalOutput(t *testing.T) {
	...
	srcs := []*txbuilder.Source{NewAccountSource(ctx, &bc.AssetAmount{AssetID: assetID, Amount: 10}, account1ID, nil, nil, nil)}
...
func transfer(ctx context.Context, t testing.TB, info *clientInfo, srcAcctID, destAcctID string, amount uint64) (*bc.Tx, error) {
	...
	source := NewAccountSource(ctx, assetAmount, srcAcctID, nil, nil, nil)
