...
func contractArgs(t testing.TB, ctx context.Context, contract compileResp, clause ivy.ClauseInfo, accounts *account.Manager, assets *asset.Registry) ([]ivy.ContractArg, map[string]interface{}) {
...
func TestContracts(t *testing.T) {
	...
	for _, test := range tests {
		compiled, err := compileIvy(compileReq{
			Contract: test.contract,
		})
		if err != nil {
			t.Error("compiling contract", err)
			continue
		}

		for i, clause := range compiled.Clauses {
			args, vals := contractArgs(t, ctx, compiled, clause, accounts, assets)
			compiled, err := compileIvy(compileReq{
				Contract: test.contract,
				Args:     args,
			})
