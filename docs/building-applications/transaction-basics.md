...
An input either issues new units of an asset, or names an output of a previous transaction as the source of asset units.

Outputs take the asset units from the inputs and define how they will be allocated. A single output can indicate an asset amount, along with a control program that specifies how that amount can be spent in the future. Or, an output can retire units of the asset, removing those units from circulation.

A transaction can have many inputs and outputs that consist of many different types of assets, many different sources, and many different destinations. All actions within a transaction (issuing, spending, controlling, and retiring assets) occur simultaneously, as a single, atomic operation.

### Asset balancing

Within the same transaction, the total amount of assets in the inputs must equal the total amount of assets in the outputs. To create new asset units, we issue in an input and control them with one or more outputs. To transfer asset units, we spend them in an input and control them with one or more outputs. To retire assets units, we spend them in an input and retire them in an output.
...
1. **Build transaction**: Define what the transaction is supposed to do: issue new units of an assets, spend assets held in an account, control assets with an account, etc.
...
Rather than forcing you to manipulate inputs, outputs and change directly, the Client API allows you to build transactions using a list of high-level **actions**.
...
| Spend from account            | Spends units of a specified asset from a specified account. Automatically handles the creation of change outputs.                            |
| Spend an unspent output from an account   | Spends an entire unspent output in an account. Change must be handled manually, using other actions.          |
...
In order for a transaction to be accepted into the blockchain, its inputs must contain valid signatures. For issuance inputs, the signature must correspond to public keys named in the issuance program. For spending inputs, the signature must correspond to the public keys named in the control programs of the output being spent.

Transaction signing provides the blockchain with its security; no one, not even the operators of the blockchain network, can provide signatures except the holders of the relevant private keys.

The Chain Core SDK assumes that private keys are held within an HSM controlled by the user. The SDK includes an `HsmSigner` interface that communicates with HSMs to sign transactions. For development, each Chain Core provides a Mock HSM that can generate public/private keypairs and sign transactions. In a production setting, the Chain Core never signs transactions.

### Submit transaction

Once a transaction is balanced and all inputs are signed, it is considered valid and can be submitted to the blockchain. The local core will forward the transaction to the generator, which adds the transaction to the blockchain and propagates it to other cores on the network.

When handling a submission, the Client API does not return a response until the transaction has been added to the blockchain and indexed by the local core, or if there was an error. This allows you to write your programs in a linear fashion. In general, if a submission responds with a success, the rest of your program may proceed with the guarantee that the transaction has been committed to the blockchain.
