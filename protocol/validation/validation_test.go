...
func TestTxValidation(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			f: func() {
				mux.Body.ExtHash = bc.NewHash([32]byte{0x01})
			},
		...
		},
		{
			...
			f: func() {
				iss := tx.TxInputs[0].(*bc.Issuance)
				nonce := iss.Anchor.(*bc.Nonce)
				nonce.Body.ExtHash = bc.NewHash([32]byte{0x01})
			},
			err: errNonemptyExtHash,
		},
		{
		...
		},
		{
			...
			f: func() {
				...
				nonce := iss.Anchor.(*bc.Nonce)
				tr := nonce.TimeRange
				tr.Body.ExtHash = bc.NewHash([32]byte{0x01})
			},
			err: errNonemptyExtHash,
		},
		{
		...
		},
		{
			...
			f: func() {
				mux.Witness.Destinations[0].Ref = bc.NewHash([32]byte{0x01})
			},
		...
		},
		{
			...
			f: func() {
				tx.Results[0].(*bc.Output).Body.ExtHash = bc.NewHash([32]byte{0x01})
			},
			err: errNonemptyExtHash,
		},
		{
		...
		},
		{
			...
			f: func() {
				tx.Body.ExtHash = bc.NewHash([32]byte{0x01})
			},
			err: errNonemptyExtHash,
		},
		{
		...
		},
		{
			...
			f: func() {
				vs.blockchainID = bc.NewHash([32]byte{0x02})
			},
		...
		},
		{
			...
			f: func() {
				tx.TxInputs[0].(*bc.Issuance).Body.ExtHash = bc.NewHash([32]byte{0x01})
			},
			err: errNonemptyExtHash,
		},
		{
		...
		},
		{
			...
			f: func() {
				tx.TxInputs[1].(*bc.Spend).Body.ExtHash = bc.NewHash([32]byte{0x01})
			},
			err: errNonemptyExtHash,
		},
		{
...
func TestBlockHeaderValid(t *testing.T) {
	...
	cases := []struct {
		...
		{
			f: func() {
				bh.Body.ExtHash = bc.NewHash([32]byte{0x01})
			},
			err: errNonemptyExtHash,
		},
	}
...
func sample(tb testing.TB, in *txFixture) *txFixture {
	...
	if (result.initialBlockID == bc.Hash{}) {
		result.initialBlockID = bc.NewHash([32]byte{0x01})
	}
	...
	if len(result.txInputs) == 0 {
		...
		result.txInputs = []*bc.TxInput{
			...
			bc.NewSpendInput(args1, bc.NewHash([32]byte{0x05}), result.assetID, 20, 0, cp1, bc.NewHash([32]byte{0x06}), []byte{7}),
			bc.NewSpendInput(args2, bc.NewHash([32]byte{0x08}), result.assetID, 40, 0, cp2, bc.NewHash([32]byte{0x9}), []byte{10}),
		}
	}
