...
func ConfirmTx(snapshot *state.Snapshot, initialBlockHash bc.Hash, block *bc.Block, tx *bc.Tx) error {
	...
	for i, txin := range tx.Inputs {
		...
		if !snapshot.Tree.Contains(k, val) {
			return badTxErrf(errInvalidOutput, "output %s for input %d is invalid", txin.Outpoint(), i)
		}
	}
	return nil
}
	...
	for i, txout := range tx.Outputs {
		if tx.Version == 1 {
			...
			if txout.VMVersion != 1 {
				return badTxErrf(errVMVersion, "unknown vm version %d in output %d for transaction version 1", txout.VMVersion, i)
			}
		}
