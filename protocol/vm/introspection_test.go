...
func TestIntrospectionOps(t *testing.T) {
	...
	cases := []testStruct{{
		...
		op: OP_ENTRYID,
		startVM: &VirtualMachine{
			Context: validation.NewTxVMContext(tx.TxEntries, tx.TxEntries.Entries[tx.TxEntries.TxInputs[0].(*bc.Spend).Witness.Destination.Ref.Hash()], &bc.Program{VmVersion: 1}, nil),
		},
		wantVM: &VirtualMachine{
			RunLimit:     49959,
			DeferredCost: 40,
			DataStack:    [][]byte{tx.TxEntries.TxInputs[0].(*bc.Spend).Witness.Destination.Ref.Hash().Bytes()[:]},
			Context:      validation.NewTxVMContext(tx.TxEntries, tx.TxEntries.Entries[tx.TxEntries.TxInputs[0].(*bc.Spend).Witness.Destination.Ref.Hash()], &bc.Program{VmVersion: 1}, nil),
		},
	}}
