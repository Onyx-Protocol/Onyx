...
func (vm *virtualMachine) run() (bool, error) {
	for vm.pc = 0; vm.pc < uint32(len(vm.curprog)); { // handle vm.pc updates in step
		err := vm.step()
		if err != nil {
			return false, err
		}
...
func (vm *virtualMachine) step() error {
	inst, err := ParseOp(vm.curprog, vm.pc)
	if err != nil {
		return err
	}
